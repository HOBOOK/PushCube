...
namespace OneStore
{
	public class Onestore_IapResultListener : MonoBehaviour
	{
		...
		{
            int productIdCheck = IAPManager.Instance.ProductIdCheck(result.productId);
            if (productIdCheck != -1)
            {
                if(productIdCheck==0)
                {
                    IAPManager.Instance.PurchaseRemoveAd();
                }
                else if(productIdCheck==1)
                {
                    IAPManager.Instance.PurchaseGoldCube();
                }
            }
