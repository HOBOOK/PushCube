...
public class AGrid : MonoBehaviour
{
    ...
    void BlurPenaltyMap(int blurSize)
    {
        ...
        int kerneExtents = (kernelSize-1)/ 2;
        ...
        for(int y = 0; y < gridSizeY; y++)
        {
            for(int x = -kerneExtents; x<=kerneExtents; x++)
            {
                int sampleX = Mathf.Clamp(x, 0, kerneExtents);
                penaltiesHorizontalPass[0, y] += grid[sampleX, y].movementPenalty;
            }
            for(int x = 1; x < gridSizeX; x++)
            {
                int removeIndex = Mathf.Clamp(x - kerneExtents - 1,0,gridSizeX);
                int addIndex = Mathf.Clamp(x + kerneExtents, 0, gridSizeX-1);
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = -kerneExtents; y <= kerneExtents; y++)
            {
                int sampleY = Mathf.Clamp(y, 0, kerneExtents);
            ...
            {
                int removeIndex = Mathf.Clamp(y - kerneExtents - 1, 0, gridSizeY);
                int addIndex = Mathf.Clamp(y + kerneExtents, 0, gridSizeY - 1);
